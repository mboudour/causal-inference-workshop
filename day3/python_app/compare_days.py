...
Day 1: Stance score, simple adjustment estimators
Day 2: Sentiment score, six estimators (DiM, Regression, G-formula, Matching, IPW, AIPW)
...
def estimate_ols(df, y_col):
    ...
    Y = df[y_col]
    D = (df['party'] == 'Republican').astype(int)
...
def main():
    ...
    if os.path.exists(day1_path):
        ...
        print(f"\nDay 1 data not found at {day1_path}. Skipping.")
    ...
    if os.path.exists(day2_path):
        ...
        print(f"Day 2 data not found at {day2_path}. Skipping.")

    # Day 3 DML
    if os.path.exists(day3_path):
        df3 = pd.read_csv(day3_path)
        dml_row = df3[df3['estimator'].str.contains('DML')].iloc[0]
        ...
        print(f"Day 3 DML results not found at {day3_path}. Run partA_dml.py first.")
